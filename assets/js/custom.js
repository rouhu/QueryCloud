        ...
        success: function(response) {
            if (response.status === 'success') {
                if (response.token) {
                    ...
                    var shareUrl = appBaseUrl + '/share/' + response.token;
                    $linkInput.val(shareUrl);
                    $copyBtn.prop('disabled', false);
                } else {
                    // No token yet, but might be generated if require_login is checked.
                    // Or if require_login is false, link remains empty until generated by checking the box.
                    $linkInput.val('No public link generated yet. Enable "Require Login" or save settings to generate.');
                }
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status === 'success') {
                ...
                $('#shareRequireLoginCheckbox').prop('checked', response.requires_login || false); // Update checkbox based on server response

                if (response.token) { // If a token was (re)generated or confirmed
                    var appBaseUrl = (base.endsWith('/') ? base.substring(0, base.length - 1) : base);
                    var shareUrl = appBaseUrl + '/share/' + response.token;
                    $linkInput.val(shareUrl);
                    $copyBtn.prop('disabled', false);
                } else if (!requireLogin) {
                    // If requireLogin is false and no token (e.g., it was cleared by backend if that logic existed, or never generated)
                    $linkInput.val('No public link generated (login not required and no active link).');
                    $copyBtn.prop('disabled', true);
                } else {
                    // If requireLogin is true but somehow no token was returned (shouldn't happen if backend generates one)
                    $linkInput.val('Link will be active once settings are fully processed.');
                    $copyBtn.prop('disabled', true);
                }
            } else {
